// Banking Cards Frontend - Clean Version
...
async function createCard(event) {
    ...
    // Get user-provided card number or generate one
    let cardNumber = document.getElementById('cardNumber').value.trim();
    if (!cardNumber) {
        // Generate dummy card number for demo
        cardNumber = '1234567890' + String(Date.now()).slice(-6);
    } else if (cardNumber.length !== 16 || !/^\d{16}$/.test(cardNumber)) {
        showToast('Номер карты должен содержать ровно 16 цифр', 'error');
        return;
    }
    
    // Send expiryDate as string in YYYY-MM format
    const cardData = {
        cardHolderName,
        cardType,
        cardNumber,
    ...
    try {
        ...
        if (response.ok) {
            ...
            showToast('Ошибка при создании карты', 'error');
        }
    } catch (error) {
...
async function toggleCardStatus(cardId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/cards/${cardId}/status?status=${newStatus}`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
...
async function loadTransactions() {
    try {
        const response = await fetch(`${API_BASE_URL}/transactions?page=0&size=50`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
...
function createTransactionElement(transaction) {
    ...
    const toCardId = transaction.toCard ? transaction.toCard.id : transaction.toCardId;
    const fromCardId = transaction.fromCard ? transaction.fromCard.id : transaction.fromCardId;
    
    const isIncoming = userCards.some(card => card.id === toCardId);
    const isOutgoing = userCards.some(card => card.id === fromCardId);
